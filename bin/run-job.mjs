...
Environment:
  ...
  PROFILESCRIBE_RIG_TOURNAMENT_MODEL   Must be qwen/qwen3.8-max when set; the pinned DeepSeek fallback is managed internally
...
async function runOpportunityDiscoveryPlanningJob(
  ...
  // classify planner_acceptance/contract recovery instead of a
  // wrapper_contract identity mismatch on status "skipped".
  return {
    status: 'completed',
    jobId: text(job.id),
    jobKind: text(job.kind),
...
function localJSONRepairRootElementShapes(error) {
  const shapes = error?.localJSONRepairRootElementShapes;
  return Array.isArray(shapes) ? shapes.slice(0, 4) : undefined;
}
